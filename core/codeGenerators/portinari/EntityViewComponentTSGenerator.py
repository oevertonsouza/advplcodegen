...
class EntityViewComponentTSGenerator(codeGenerator):

    def __init__ (self, entity=None):
        super().__init__(entity=None)
        self.templateFile = 'entity-view.component.ts.template'
    ...
    def getVariables(self):
        ...
        fields = ''
        descriptionPath = ''
        sufixFileName = '-view.component.ts'
        ...
        for entity in Entity.select():
            ...
            for column in Colunas.select().join(Entity).where(Entity.table == self.entity.table):
                fields += ''.rjust(4) + "{ property: '" + column.name + "', label: '" + column.desc + "', disabled: true " 
