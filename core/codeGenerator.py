        ...
        if exists:
            with open(storagePathFile) as datafile:
                ...
                for column in columnInfo:
                    properts += '    Method get'+ column[1].replace("_", "").capitalize() + '()\n'
                    gets     += (
                                    'Method get'+ column[2].replace("_", "").capitalize() + '() Class '+ name + '\n'
                                    'Return self:getValue("' + column[1].replace("_", "").lower() + '")\n\n'
                                )
                    serialize   += '    oJsonControl:setProp(oJson,"' + column[1].replace("_", "").lower() + '",self:get'+ column[1].replace("_", "").capitalize() +'())\n'
                    fields      += '    self:oFields:push({"'+column[1].replace("_", "").capitalize()+'", self:get'+ column[1].replace("_", "").capitalize() +'()})\n'
